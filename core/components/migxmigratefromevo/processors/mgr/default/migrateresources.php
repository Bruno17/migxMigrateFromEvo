    ...
    foreach ($collection as $source_object) {
        ...
        if ($object = $modx->newObject($target_classname)) {
            ...
            if ($convert) {
                ...
                $translator->translate($item['content']);
                $translator->translate($item['pagetitle']);
                $translator->translate($item['longtitle']);
                $translator->translate($item['introtext']);
                $translator->translate($item['description']);
                $translator->translate($item['menutitle']);
            }
